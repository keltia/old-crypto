...
#
# $Id: key.rb,v f7f81f5c1ec6 2009/02/20 23:37:38 roberto $
...
# == class SQKey
#
# Class for handling keys generated by a Polybius square.  For
#
class SQKey < SKey
  ...
  CODE_WORD = {
    SQ_NUMBERS5 => [ 0, 1, 2, 3, 4 ],
    SQ_NUMBERS6 => [ 0, 1, 2, 3, 4, 5 ],
    SQ_ADFGX    => [ 'A', 'D', 'F', 'G', 'X' ],
  ...
  def initialize(key, type = SQ_NUMBERS6)
    super(key)
    @alpha = Hash.new
    @ralpha = Hash.new
    @type = type
    if @type == SQ_NUMBERS5 or @type == SQ_ADFGX then
      @base = BASE25
    else
      @base = BASE36
    end
